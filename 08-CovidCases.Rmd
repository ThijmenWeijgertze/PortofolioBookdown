...
covidDataMatrixCases <- covidDataFil %>% mutate(Normalized = cases/popData2020) %>%  select(countriesAndTerritories, day, Normalized) 
...
# https://stackoverflow.com/questions/14123792/drop-true-doesnt-drop-factor-levels-in-data-frame-while-in-vector-it-does
...
```{r}
# matrix preparation for cases
covidDataMatrixDeaths <- covidDataFil %>% mutate(Normalized = deaths/popData2020) %>%  select(countriesAndTerritories, day, Normalized) 
